class GrowSystemPanel extends HTMLElement {
  ...
  _activeStageSummary() {
    ...
    return `<ha-card class="active-stage-card"><div class="active-stage-identity"><span class="eyebrow">Aktif aşama</span><div><ha-icon icon="${icons[stage]}"></ha-icon><span><b>${labels[stage]}</b><small>${p.photoperiod} saat açık · ${24-p.photoperiod} saat karanlık</small></span></div></div><div class="active-stage-targets"><span><small>Işık</small><b>${p.light_intensity}%</b></span><span><small>Sıcaklık</small><b>${p.day_temperature}° / ${p.night_temperature}°C</b></span><span><small>Atmosfer</small><b>${p.humidity}% · ${p.vpd} kPa · ${p.co2} ppm</b></span><span><small>Kök bölgesi</small><b>${p.ppm} ppm · pH ${p.ph} · ${p.water_temperature}°C</b></span></div><ha-button data-edit-active-stage appearance="plain">Profili düzenle<ha-icon icon="mdi:chevron-right"></ha-icon></ha-button></ha-card>`;
  }
  ...
  _cycleSummary(){
    ...
    return `<ha-card class="cycle-summary"><div><ha-icon icon="mdi:sprout-outline"></ha-icon><span><b>${c.name}</b><small>${c.start_date} tarihinde başladı · Gün ${current}/${total}</small></span></div><ha-button data-open-today appearance="plain">Bugünü aç</ha-button></ha-card>`;
  }
  _calendarView(){
  ...
  _profiles() {
    const order=["germination","early_veg","veg","bloom","darkness"],labels={germination:"Çimlenme",early_veg:"Erken veg",veg:"Veg",bloom:"Çiçeklenme",darkness:"Karanlık"},icons={germination:"mdi:sprout",early_veg:"mdi:leaf",veg:"mdi:flower",bloom:"mdi:flower-pollen",darkness:"mdi:weather-night"};
    const cultivationActive=Boolean(this._config?.cultivation?.active);
    return `<ha-card class="profile-list-card"><div class="profile-matrix-head"><div><span class="eyebrow">Aşama hedefleri</span><h2>Profiller</h2><p>Bir profili açarak hedeflerini güvenli bir ekranda düzenleyin.</p></div><span data-status>${this._notice}</span></div><div class="profile-list">${order.map((stage)=>{const p=this._profileDrafts[stage],active=cultivationActive&&stage===this._config.active_stage;return `<button class="profile-list-row ${active?"active":""}" data-open-profile="${stage}"><ha-icon icon="${icons[stage]}"></ha-icon><span class="profile-list-name"><b>${labels[stage]}</b><small>${active?"Aktif aşama":"Hazır profil"}</small></span><span class="profile-summary"><em><small>Işık</small>${p.photoperiod} sa · ${p.light_intensity}%</em><em><small>Gündüz / gece</small>${p.day_temperature}° / ${p.night_temperature}°</em><em><small>Atmosfer</small>${p.humidity}% · ${p.vpd} kPa · ${p.co2} ppm</em><em><small>Kök bölgesi</small>${p.ppm} ppm · pH ${p.ph} · ${p.water_temperature}°C</em></span><ha-icon class="profile-chevron" icon="mdi:chevron-right"></ha-icon></button>`;}).join("")}</div></ha-card>`;
  }
  ...
  async _saveProfileRow(stage){
    ...
    try{const saved=await this._hass.connection.sendMessagePromise({type:"grow_system/profile/save",stage,values:this._profileDrafts[stage]});this._config.profiles[stage]=saved;this._profileDrafts[stage]={...saved};if(stage===this._editingStage)this._draft={...saved};this._notice=`${saved.name} kaydedildi`;}
