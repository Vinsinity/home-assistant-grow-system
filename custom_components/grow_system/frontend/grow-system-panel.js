class GrowSystemPanel extends HTMLElement {
  ...
  _profiles() {
    ...
    return `<ha-card class="profile-matrix-card"><div class="profile-matrix-head"><div><span class="eyebrow">Aşama hedefleri</span><h2>Tüm profiller</h2><p>Her satır bağımsız kaydedilir. Aktif profil otomatik kontrol kapalıyken yalnızca güncel hedefleri belirler.</p></div><span data-status>${this._notice}</span></div><div class="profile-table-scroll"><table class="profile-table"><thead><tr><th>Aşama</th><th>Işık</th><th>Sıcaklık</th><th>Atmosfer</th><th>Kök bölgesi</th><th></th></tr></thead><tbody>${order.map((stage)=>`<tr class="${stage===this._config.active_stage?"active":""}"><th><ha-icon icon="${icons[stage]}"></ha-icon><span><b>${labels[stage]}</b><small>${stage===this._config.active_stage?"Aktif profil":"Profil"}</small></span></th><td><div class="profile-cell two">${this._profileInput(stage,"photoperiod","Süre","sa",0,24,1)}${this._profileInput(stage,"light_intensity","Şiddet","%",0,100,5)}</div></td><td><div class="profile-cell two">${this._profileInput(stage,"day_temperature","Gündüz","°C",10,35,.5)}${this._profileInput(stage,"night_temperature","Gece","°C",10,35,.5)}</div></td><td><div class="profile-cell three">${this._profileInput(stage,"humidity","Nem","%",30,90,1)}${this._profileInput(stage,"vpd","VPD","kPa",.2,2.5,.05)}${this._profileInput(stage,"co2","CO₂","ppm",350,1500,25)}</div></td><td><div class="profile-cell four">${this._profileInput(stage,"ppm","Besin","ppm",0,2000,10)}${this._profileInput(stage,"water_temperature","Su","°C",10,30,.5)}${this._profileInput(stage,"ph","pH","pH",4,8,.1)}${this._profileInput(stage,"do_minimum","DO min","mg/L",0,15,.1)}</div></td><td><div class="profile-row-actions"><ha-button data-save-profile-row="${stage}" appearance="filled">Kaydet</ha-button><ha-button data-activate-profile="${stage}" appearance="plain" ${stage===this._config.active_stage?"disabled":""}>Aktif yap</ha-button></div></td></tr>`).join("")}</tbody></table></div></ha-card>`;
  }
