class GrowSystemPanel extends HTMLElement {
  ...
  _hardwareCard() {
    ...
    const candidates=discovered.filter((candidate)=>!assignments.some((item)=>Number(item.address)===parseInt(candidate.address,16))).map((candidate)=>`<button class="candidate" data-candidate-address="${candidate.address}" data-candidate-driver="${candidate.suggested_driver}" data-candidate-name="${candidate.chip}"><code>${candidate.address}</code><span><b>${candidate.chip}</b><small>${candidate.firmware?`Firmware ${candidate.firmware}`:"Sürücü seçilmedi"}</small></span><ha-icon icon="mdi:chevron-right"></ha-icon></button>`).join("");
    return `<ha-card class="hardware-card"><div class="i2c-heading"><div><span class="eyebrow">Raspberry Pi · ${hw.path || "/dev/i2c-1"}</span><h2>Yerel I²C donanımı</h2><p>${hw.available ? "Veri yolu hazır; keşif yalnızca okuma yapıyor." : (hw.error || "I²C veri yolu kullanılamıyor.")}</p></div><div class="bus-health ${hw.available ? "online" : "offline"}"><span></span>${hw.available ? "Çevrimiçi" : "Çevrimdışı"}</div></div><div class="i2c-workspace">
      ...
      <aside class="device-adder"><span class="eyebrow">Cihaz ekle</span><h3>Keşfedilen cihazlar</h3><p>Keşif yalnızca adayları gösterir. Siz eklemeden hiçbir cihaz etkinleştirilmez.</p><div class="candidate-list">${candidates||'<div class="candidate-empty">Eklenmemiş yeni cihaz bulunamadı.</div>'}</div><div class="manual-divider"><span>veya adresi elle girin</span></div><ha-textfield data-new-address label="I²C adresi" value="0x40"></ha-textfield><ha-textfield data-new-name label="Cihaz adı" value=""></ha-textfield><ha-select data-new-driver label="Sürücü / model"><mwc-list-item value="waveshare_motor_hat">Waveshare Motor Driver HAT</mwc-list-item><mwc-list-item value="atlas_do">Atlas EZO DO</mwc-list-item><mwc-list-item value="atlas_ph">Atlas EZO pH</mwc-list-item><mwc-list-item value="atlas_ec">Atlas EZO EC</mwc-list-item><mwc-list-item value="atlas_rtd">Atlas EZO RTD</mwc-list-item><mwc-list-item value="pca9685_generic">Genel PCA9685</mwc-list-item></ha-select><div class="driver-note"><ha-icon icon="mdi:information-outline"></ha-icon><span data-driver-note>İki çıkış: Motor A ve Motor B. Adres aralığı 0x40–0x5F.</span></div><ha-button data-add-i2c appearance="filled">Cihazı ekle</ha-button></aside>
  ...
  _wireHardware() {
    ...
    this.shadowRoot.querySelectorAll("[data-candidate-address]").forEach((button)=>button.addEventListener("click",()=>{this.shadowRoot.querySelector("[data-new-address]").value=button.dataset.candidateAddress;this.shadowRoot.querySelector("[data-new-name]").value=button.dataset.candidateName;this._newI2CDriver=button.dataset.candidateDriver;driver.value=this._newI2CDriver;this._updateDriverNote();}));
  ...
  _addI2CDevice(){
    ...
    this._hardwareDraft.device_assignments.push({address,driver:this._newI2CDriver,name:name||defaults[this._newI2CDriver]});
