class GrowSystemPanel extends HTMLElement {
  ...
  _fluidCatalogForDialog(){
    ...
    const isDown=(product)=>/(down|min|minus|lemon|ph−|ph-)/i.test(`${product[0]} ${product[1]}`);
    return catalog.map((brand)=>brand.id==="custom"?brand:{...brand,products:brand.products.filter((product)=>product[2]==="ph"&&(down?isDown(product):!isDown(product)))}).filter((brand)=>brand.id==="custom"||brand.products.length);
  }
  _fluidDialogView(){
