class GrowSystemPanel extends HTMLElement {
  ...
  _nutrientCatalog(){return [
    {id:"advanced_nutrients",name:"Advanced Nutrients",products:[
      ["sensi_grow_a","pH Perfect Sensi Grow A","base"],["sensi_grow_b","pH Perfect Sensi Grow B","base"],["sensi_bloom_a","pH Perfect Sensi Bloom A","base"],["sensi_bloom_b","pH Perfect Sensi Bloom B","base"],["gmb_grow","pH Perfect Grow","base"],["gmb_micro","pH Perfect Micro","base"],["gmb_bloom","pH Perfect Bloom","base"],["big_bud","Big Bud","booster"],["bud_candy","Bud Candy","supplement"],["b52","B-52","supplement"],["calmag_xtra","Sensi Cal Mag Xtra","supplement"],["overdrive","Overdrive","booster"],["flawless_finish","Flawless Finish","cleaner"]]},
    {id:"general_hydroponics",name:"General Hydroponics",products:[
      ["flora_gro","FloraGro","base"],["flora_micro","FloraMicro","base"],["flora_bloom","FloraBloom","base"],["calimagic","CALiMAGic","supplement"],["rapidstart","RapidStart","supplement"],["liquid_koolbloom","Liquid KoolBloom","booster"],["florakleen","FloraKleen","cleaner"]]},
    {id:"terra_aquatica",name:"Terra Aquatica",products:[
      ["tripart_grow","TriPart Grow","base"],["tripart_micro_soft","TriPart Micro Soft Water","base"],["tripart_micro_hard","TriPart Micro Hard Water","base"],["tripart_bloom","TriPart Bloom","base"],["pro_roots","Pro Roots","supplement"],["seaweed","Seaweed","supplement"],["humic","Humic","supplement"],["pro_bloom","Pro Bloom","booster"],["flashclean","FlashClean","cleaner"],["finalpart","FinalPart","booster"]]},
  ...
  _dosingFluidsCard(){
    ...
    return `<ha-card class="dosing-card"><div class="dosing-head"><div><span class="eyebrow">Dozaj tesisatı</span><h2>Dozaj sıvıları ve besinler</h2><p>Marka ve ürünü katalogdan seçin veya kendi ürününüzü tanımlayın.</p></div><ha-button data-open-fluid-dialog appearance="filled"><ha-icon icon="mdi:plus"></ha-icon>Besin ekle</ha-button></div><div class="fluid-list">${fluids.map((fluid)=>`<div class="fluid-row"><ha-icon icon="${fluid.required?"mdi:flask":"mdi:bottle-tonic-plus-outline"}"></ha-icon><span><b>${fluid.name}</b><small>${fluid.required?"Zorunlu sistem sıvısı":`${fluid.brand||"Özel"} · ${categoryLabels[fluid.category]||"Diğer"}`}</small></span>${fluid.required?'<span class="required-badge">Zorunlu</span>':`<ha-icon-button data-remove-fluid="${fluid.id}" title="Besini kaldır"><ha-icon icon="mdi:delete-outline"></ha-icon></ha-icon-button>`}</div>`).join("")}</div></ha-card>`;
  }
  _fluidDialogView(){
    if(!this._fluidDialog)return "";
    const catalog=this._nutrientCatalog(),brand=catalog.find((item)=>item.id===this._fluidBrand)||catalog[0];
    const product=brand.products.find((item)=>item[0]===this._fluidProduct)||brand.products[0];
    const custom=brand.id==="custom";
    const categoryLabels={base:"Baz besin",supplement:"Takviye",booster:"Çiçeklenme desteği",cleaner:"Sistem temizleyici",ph:"pH düzenleyici",other:"Diğer"};
    return `<div class="dialog-scrim" data-fluid-scrim><div class="device-dialog fluid-dialog" role="dialog" aria-modal="true" aria-label="Besin ekle"><div class="dialog-head"><div><span class="eyebrow">Besin kataloğu</span><h2>Dozaj sıvısı ekle</h2></div><ha-icon-button data-close-fluid-dialog><ha-icon icon="mdi:close"></ha-icon></ha-icon-button></div><div class="dialog-body"><div class="catalog-fields"><ha-selector data-fluid-brand></ha-selector>${custom?`<ha-textfield data-custom-brand label="Marka" placeholder="Üretici veya marka adı"></ha-textfield><ha-textfield data-custom-product label="Ürün adı" placeholder="Besin veya katkı adı"></ha-textfield><ha-selector data-custom-category></ha-selector>`:`<ha-selector data-fluid-product></ha-selector>`}</div>${product&&!custom?`<div class="product-preview"><ha-icon icon="mdi:bottle-tonic-plus-outline"></ha-icon><span><small>${brand.name}</small><b>${product[1]}</b><em>${categoryLabels[product[2]]}</em></span></div>`:""}<div class="device-note"><ha-icon icon="mdi:information-outline"></ha-icon><span><b>Her şişeyi ayrı ekleyin</b><small>A/B veya Grow/Micro/Bloom sistemlerinde her dozaj pompası için ilgili ürünü ayrı kaydedin.</small></span></div><div class="device-message">${this._deviceNotice}</div></div><div class="dialog-actions"><ha-button data-close-fluid-dialog appearance="plain">Vazgeç</ha-button><ha-button data-confirm-fluid appearance="filled">Besini ekle</ha-button></div></div></div>`;
  }
  ...
  _wireDosing(){
    this.shadowRoot.querySelector("[data-open-fluid-dialog]")?.addEventListener("click",()=>{this._fluidDialog=true;this._deviceNotice="";const brand=this._nutrientCatalog().find((item)=>item.id===this._fluidBrand);this._fluidProduct=brand?.products?.[0]?.[0]||"";this._render();});
  ...
  _addCatalogFluid(){
    const catalog=this._nutrientCatalog(),brand=catalog.find((item)=>item.id===this._fluidBrand)||catalog[0],custom=brand.id==="custom";
    const product=brand.products.find((item)=>item[0]===this._fluidProduct)||brand.products[0];
    ...
    const category=custom?(this.shadowRoot.querySelector("[data-custom-category]")?.value||"other"):product?.[2];
    if(!brandName||!name){this._deviceNotice="Marka ve ürün adını doldurun.";this._render();return;}
    this._hardwareDraft.dosing_fluids||=[];let id=this._fluidId(`${brand.id}_${name}`),suffix=2;while(this._hardwareDraft.dosing_fluids.some((fluid)=>fluid.id===id))id=`${this._fluidId(`${brand.id}_${name}`)}_${suffix++}`;
    this._hardwareDraft.dosing_fluids.push({id,name,brand:brandName,category:category||"other",catalog_id:custom?"":`${brand.id}:${product[0]}`,required:false});this._fluidDialog=false;this._saveHardware();
  }
