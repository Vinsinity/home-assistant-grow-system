class GrowSystemPanel extends HTMLElement {
  ...
  _fluidCatalogForDialog(){
    ...
    return catalog.map((brand)=>brand.id==="custom"?brand:{...brand,products:brand.products.filter((product)=>product[2]==="ph"&&(down?product[1].toLowerCase().includes("down"):!product[1].toLowerCase().includes("down")))}).filter((brand)=>brand.id==="custom"||brand.products.length);
  }
  _fluidDialogView(){
  ...
  _addCatalogFluid(){
    ...
    if(this._editingFluidId){const index=this._hardwareDraft.dosing_fluids.findIndex((fluid)=>fluid.id===this._editingFluidId);this._hardwareDraft.dosing_fluids[index]={...this._hardwareDraft.dosing_fluids[index],name,brand:brandName,category:category||"other",catalog_id:custom?"":`${brand.id}:${product[0]}`};}
    else{let id=this._fluidId(`${brand.id}_${name}`),suffix=2;while(this._hardwareDraft.dosing_fluids.some((fluid)=>fluid.id===id))id=`${this._fluidId(`${brand.id}_${name}`)}_${suffix++}`;this._hardwareDraft.dosing_fluids.push({id,name,brand:brandName,category:category||"other",catalog_id:custom?"":`${brand.id}:${product[0]}`,required:false});}
