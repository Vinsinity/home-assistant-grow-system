class GrowSystemPanel extends HTMLElement {
  ...
  _profileEditorDialog(){
    ...
    return `<div class="dialog-scrim" data-profile-scrim><div class="device-dialog profile-editor-dialog" role="dialog" aria-modal="true"><div class="dialog-head"><div><span class="eyebrow">Yetiştirme profili</span><h2>${labels[stage]}</h2></div><ha-icon-button data-close-profile><ha-icon icon="mdi:close"></ha-icon></ha-icon-button></div><div class="dialog-body profile-editor-body">
      ...
      <div class="profile-editor-note" data-status>${this._notice}</div></div><div class="dialog-actions profile-editor-actions">${active?'<span class="active-profile-label"><ha-icon icon="mdi:check-circle"></ha-icon>Aktif profil</span>':`<ha-button data-activate-profile="${stage}" appearance="plain">Aktif profil yap</ha-button>`}<ha-button data-save-profile-row="${stage}" appearance="filled">Değişiklikleri kaydet</ha-button></div></div></div>`;
  }
  ...
  async _activateProfile(stage){
    ...
    await this._hass.connection.sendMessagePromise({type:"grow_system/stage/select",stage});this._config.active_stage=stage;this._editingStage=stage;this._draft={...this._config.profiles[stage]};this._render();
  }
