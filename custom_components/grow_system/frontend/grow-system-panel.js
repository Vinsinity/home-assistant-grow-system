class GrowSystemPanel extends HTMLElement {
  ...
  _securityOverview() {
    ...
    return `<ha-card class="security-card"><div class="security-head"><div><ha-icon icon="mdi:shield-home-outline"></ha-icon><span><b>Güvenlik</b><small>${alarm ? "Su baskını alarmı var" : "Tüm sensörler normal"}</small></span></div>${alarm ? '<strong><ha-icon icon="mdi:alert"></ha-icon>Alarm</strong>' : ""}</div>
      ...
      <section class="security-zone"><header><ha-icon icon="mdi:waves-arrow-up"></ha-icon><span><b>RDWC su seviyesi</b><small>${waterLevel ? "Canlı seviye bilgisi" : "Sensör eklenmedi"}</small></span></header><div class="water-level ${waterLevel ? "" : "unknown"}"><ha-icon icon="mdi:waves-arrow-up"></ha-icon><span><b>${waterLevel ? (waterLevel.attributes.friendly_name || waterLevelId) : "RDWC su seviyesi"}</b><small>${waterLevel ? "Güncel okuma" : "Sensör eşleştirilmedi"}</small></span><strong>${waterLevel ? `${waterLevel.state}${waterLevel.attributes.unit_of_measurement ? ` ${waterLevel.attributes.unit_of_measurement}` : ""}` : "—"}</strong></div></section></div>
    </ha-card>`;
  }
