class GrowSystemPanel extends HTMLElement {
  ...
  _cultivationDays(){
    ...
    for(const block of cultivation.plan||[]){for(let index=0;index<Number(block.planned_days||0);index++){
      const current=new Date(start);current.setDate(start.getDate()+offset);const iso=current.toISOString().slice(0,10);
  ...
  _cycleSummary(){
    ...
    if(!c.active)return `<ha-card class="cycle-summary empty"><div><ha-icon icon="mdi:calendar-seed-outline"></ha-icon><span><b>Henüz bir yetiştirme başlatılmadı</b><small>Takvim, günlük ölçümler ve fotoğraflar tek döngü altında tutulacak.</small></span></div><ha-button data-start-cultivation appearance="filled">Yetiştirmeyi başlat</ha-button></ha-card>`;
    ...
    const progress=this._cultivationProgress();
    return `<ha-card class="cycle-summary"><div><ha-icon icon="mdi:sprout-outline"></ha-icon><span><b>${c.name}</b><small>Toplam gün ${current}/${total} · Aşama günü ${progress.stageDay}/${progress.stagePlanned}</small></span></div><ha-button data-open-today appearance="plain">Bugünü aç</ha-button></ha-card>`;
  }
  _calendarView(){
    const c=this._config?.cultivation||{},today=new Date().toISOString().slice(0,10),growDays=this._cultivationDays(),growByDate=Object.fromEntries(growDays.map((day)=>[day.date,day]));
    ...
    return `<div class="journal-view">${this._cycleSummary()}<ha-card class="calendar-card"><div class="calendar-head"><div><span class="eyebrow">Sistem günlüğü</span><h2>${month.toLocaleDateString("tr-TR",{month:"long",year:"numeric"})}</h2><p>Takvim her zaman çalışır; aktif yetiştirme varsa gün ve aşama bilgisi üzerine işlenir.</p></div><div class="month-nav"><ha-icon-button data-calendar-prev title="Önceki ay"><ha-icon icon="mdi:chevron-left"></ha-icon></ha-icon-button><ha-button data-calendar-today appearance="plain">Bugün</ha-button><ha-icon-button data-calendar-next title="Sonraki ay"><ha-icon icon="mdi:chevron-right"></ha-icon></ha-icon-button></div></div>${c.active?`<div class="calendar-legend"><span class="germination">Çimlenme</span><span class="early_veg">Erken gelişim</span><span class="veg">Gelişim</span><span class="bloom">Çiçeklenme</span><span class="darkness">Karanlık</span><span class="harvest">Hasat / Kurutma</span></div>`:""}<div class="weekday"><span>Pzt</span><span>Sal</span><span>Çar</span><span>Per</span><span>Cum</span><span>Cmt</span><span>Paz</span></div><div class="grow-calendar">${cells.map((cell)=>`<button data-grow-day="${cell.iso}" class="grow-day ${cell.grow?.stage||"plain"} ${cell.current?"":"outside"} ${cell.iso===today?"today":""} ${cell.grow?.actual&&cell.grow.actual!==cell.grow.stage?"deviation":""}">${cell.grow?`<small>Yetiştirme günü ${cell.grow.number}</small>`:""}<b>${cell.date.getDate()}</b><span>${cell.grow?.label||""}</span>${cell.journal.photos?.length?`<em><ha-icon icon="mdi:image-multiple-outline"></ha-icon>${cell.journal.photos.length}</em>`:""}</button>`).join("")}</div></ha-card></div>`;
  }
  _albumView(){
    ...
    return `<div class="journal-view">${this._cycleSummary()}<ha-card class="album-card"><div class="calendar-head"><div><span class="eyebrow">Görsel günlük</span><h2>Albüm</h2><p>Fotoğraflar takvim tarihine bağlıdır; yetiştirme varsa gün numarası da gösterilir.</p></div></div>${photos.length?`<div class="album-grid">${photos.map((photo)=>`<button data-grow-day="${photo.date}"><img src="${photo.url}" alt="${photo.day?`Yetiştirme günü ${photo.day}`:photo.date}"><span>${photo.day?`Yetiştirme günü ${photo.day} · `:""}${photo.date}</span></button>`).join("")}</div>`:`<div class="album-empty"><ha-icon icon="mdi:image-multiple-outline"></ha-icon><b>Albüm henüz boş</b><span>Gün detayından fotoğraf eklendiğinde burada kronolojik olarak görünecek.</span></div>`}</ha-card></div>`;
  }
  _growDayDialog(){
    ...
    return `<div class="dialog-scrim" data-day-scrim><div class="device-dialog day-dialog"><div class="dialog-head"><div><span class="eyebrow">${day?`Yetiştirme günü ${day.number}`:"Takvim günü"}</span><h2>${title}</h2></div><ha-icon-button data-close-day><ha-icon icon="mdi:close"></ha-icon></ha-icon-button></div><div class="dialog-body">${day?`<div class="day-status"><span><small>Planlanan aşama</small><b>${day.label}</b></span><span><small>Gerçek aşama</small><b>${day.actual||"Henüz kaydedilmedi"}</b></span></div>`:""}<div class="day-section"><h3>Günün ölçümleri</h3><p>Bu tarih için Home Assistant Recorder geçmişi grafiklere bağlanacak.</p></div><div class="day-section"><h3>Fotoğraflar ve notlar</h3><p>${journal.photos?.length?`${journal.photos.length} fotoğraf kayıtlı.`:"Bu güne henüz fotoğraf veya not eklenmedi."}</p></div></div></div></div>`;
  }
  ...
  _settingsView() {
    ...
    return `<div class="settings-grid"><ha-card class="settings-card">${head("Ölçüm","Ortam sensörleri","mdi:home-thermometer-outline")}<div class="card-content settings-list">
      ...
      ${this._selector("Besin PPM sensörü","ppm_sensor")}${this._selector("pH sensörü","ph_sensor")}${this._selector("Çözünmüş oksijen sensörü","do_sensor")}${this._selector("Su sıcaklığı sensörleri","water_temperature_sensor")}${this._selector("Su seviye sensörleri","water_level_sensor")}
  ...
  _deviceSettingsDialog() {
    ...
    const motorBody=motor?`<div class="device-note safe"><ha-icon icon="mdi:shield-check-outline"></ha-icon><span><b>Güvenli pompa kalibrasyonu</b><small>Motor yalnızca onaylanan test süresince çalışır ve süre sonunda zorunlu olarak durur. Otomatik dozaj, kanal kalibre edilmeden açılamaz.</small></span></div><div class="motor-channels">${channels.map((channel,index)=>{const selected=channel.fluid_id||channel.role||"unassigned",cal=channel.calibration;return `<section class="motor-channel"><div class="motor-title"><b>Motor ${channel.id}</b><small>${channel.id==="A"?"PWM 0 · DIR 1/2":"PWM 5 · DIR 3/4"}</small>${cal?`<em class="calibrated">${Number(cal.flow_ml_s).toFixed(3)} ml/sn · %${cal.speed}</em>`:'<em class="not-calibrated">Kalibre edilmedi</em>'}</div><ha-textfield data-channel-name="${index}" label="Kanal adı" value="${channel.name}"></ha-textfield><label class="channel-fluid-field"><span>Bağlı dozaj sıvısı</span><select data-channel-fluid="${index}"><option value="unassigned" ${selected==="unassigned"?"selected":""}>Bağlantı atanmadı</option>${fluids.map((fluid)=>`<option value="${fluid.id}" ${selected===fluid.id?"selected":""}>${fluid.name}${fluid.brand?` · ${fluid.brand}`:""}</option>`).join("")}</select></label><div class="pump-calibration"><ha-textfield data-motor-seconds="${index}" label="Test süresi" suffix="sn" type="number" min="1" max="30" value="${cal?.seconds||5}"></ha-textfield><ha-textfield data-motor-speed="${index}" label="Hız" suffix="%" type="number" min="20" max="100" value="${cal?.speed||100}"></ha-textfield><ha-button data-run-motor="${index}" appearance="outlined">Pompayı test et</ha-button><ha-textfield data-motor-volume="${index}" label="Ölçülen hacim" suffix="ml" type="number" min="0.1" max="500" step="0.1" value="${cal?.volume_ml||""}"></ha-textfield><ha-button data-save-motor-cal="${index}" appearance="filled">Kalibrasyonu kaydet</ha-button></div></section>`;}).join("")}</div>`:"";
  ...
  async _activateProfile(stage){
    ...
    const profile=this._profileDrafts[stage];if(stage===this._config.active_stage||!confirm(`${profile.name} aktif profil yapılsın mı? Otomatik kontrol kapalı kalacak.`))return;
  ...
  _render() {
    ...
    const body=this._tab==="overview"?`${this._cycleSummary()}${this._securityOverview()}${this._overview()}`:this._tab==="profiles"?this._profiles():this._tab==="calendar"?this._calendarView():this._tab==="album"?this._albumView():this._tab==="hardware"?this._hardwareView():this._tab==="nutrients"?this._nutrientsView():this._settingsView();
    ...
    this.shadowRoot.querySelector("[data-open-today]")?.addEventListener("click",()=>{this._selectedGrowDay=new Date().toISOString().slice(0,10);this._render();});
    ...
    this.shadowRoot.querySelectorAll("[data-grow-day]").forEach((button)=>button.addEventListener("click",()=>{this._selectedGrowDay=button.dataset.growDay;this._render();}));
    this.shadowRoot.querySelectorAll("[data-close-day]").forEach((button)=>button.addEventListener("click",()=>{this._selectedGrowDay=null;this._render();}));
    this.shadowRoot.querySelector("[data-day-scrim]")?.addEventListener("click",(event)=>{if(event.target===event.currentTarget){this._selectedGrowDay=null;this._render();}});
  ...
  static get styles(){return `
    ...
    .profile-list-card{overflow:hidden}.profile-list{display:grid}.profile-list-row{display:grid;grid-template-columns:34px minmax(150px,.7fr) minmax(0,2.8fr) 24px;align-items:center;gap:16px;min-height:88px;padding:14px 24px;border:0;border-bottom:1px solid var(--divider-color);color:var(--primary-text-color);background:transparent;text-align:left;cursor:pointer}.profile-list-row:last-child{border-bottom:0}.profile-list-row:hover{background:var(--secondary-background-color)}.profile-list-row.active{background:color-mix(in srgb,var(--primary-color) 6%,var(--card-background-color))}.profile-list-row>ha-icon{--mdc-icon-size:25px;color:var(--state-icon-color)}.profile-list-row.active>ha-icon{color:var(--primary-color)}.profile-list-name b,.profile-list-name small{display:block}.profile-list-name b{font-size:15px}.profile-list-name small{margin-top:4px;color:var(--secondary-text-color);font-size:10px}.profile-list-row.active .profile-list-name small{color:var(--primary-color)}.profile-summary{display:grid;grid-template-columns:.65fr .85fr 1.25fr 1.25fr;gap:12px}.profile-summary em,.profile-summary small{display:block}.profile-summary em{color:var(--primary-text-color);font-size:11px;font-style:normal}.profile-summary small{margin-bottom:5px;color:var(--secondary-text-color);font:500 9px ui-monospace,SFMono-Regular,Menlo,monospace;text-transform:uppercase}.profile-chevron{--mdc-icon-size:19px;color:var(--secondary-text-color)}.profile-editor-dialog{width:min(820px,100%)}.profile-editor-body{grid-template-columns:1fr 1fr}.profile-editor-body section{display:grid;align-content:start;gap:14px;padding:16px;border:1px solid var(--divider-color);border-radius:9px}.profile-editor-body section>header{display:flex;align-items:center;gap:10px}.profile-editor-body section>header>ha-icon{color:var(--state-icon-color)}.profile-editor-body section>header span,.profile-editor-body section>header small{display:block}.profile-editor-body section>header b{font-size:14px}.profile-editor-body section>header small{margin-top:3px;color:var(--secondary-text-color);font-size:10px}.profile-editor-note{grid-column:1/-1;min-height:18px;color:var(--secondary-text-color);font-size:11px}.profile-editor-actions{min-height:42px}.profile-editor-actions>span,.active-profile-label{display:flex;align-items:center;gap:7px;margin-right:auto;color:var(--success-color,#43a047);font-size:12px}.active-profile-label ha-icon{--mdc-icon-size:18px}
    @media(max-width:900px){.profile-list-row{grid-template-columns:30px 130px minmax(0,1fr) 20px}.profile-summary{grid-template-columns:1fr 1fr}.profile-editor-body{grid-template-columns:1fr}}@media(max-width:650px){.profile-list-row{grid-template-columns:28px minmax(0,1fr) 20px;padding:14px 16px}.profile-summary{grid-column:2;grid-template-columns:1fr 1fr}.profile-chevron{grid-column:3;grid-row:1}.profile-editor-body .profile-cell.three,.profile-editor-body .profile-cell.four{grid-template-columns:1fr 1fr}.profile-editor-actions{align-items:stretch;flex-direction:column-reverse}.profile-editor-actions>span,.active-profile-label{margin:0}}
    ...
    .device-dialog:has(.motor-channels){width:min(980px,100%)}.motor-channel{grid-template-columns:120px minmax(150px,.7fr) minmax(190px,1fr)!important}.motor-title em{display:block;width:max-content;margin-top:7px;padding:3px 7px;border-radius:10px;font-size:10px;font-style:normal}.motor-title .calibrated{color:var(--success-color,#43a047);background:color-mix(in srgb,var(--success-color,#43a047) 12%,transparent)}.motor-title .not-calibrated{color:var(--warning-color,#ff9800);background:color-mix(in srgb,var(--warning-color,#ff9800) 12%,transparent)}.pump-calibration{grid-column:1/-1;display:grid;grid-template-columns:120px 120px auto 150px auto;align-items:end;gap:10px;padding-top:12px;border-top:1px solid var(--divider-color)}.dosing-readiness{display:flex;align-items:center;gap:11px;margin-top:16px;padding:13px;border:1px solid var(--divider-color);border-radius:9px}.dosing-readiness span,.dosing-readiness small{display:block}.dosing-readiness small{margin-top:3px;color:var(--secondary-text-color);font-size:11px}.dosing-readiness.ready ha-icon{color:var(--success-color,#43a047)}.dosing-readiness.attention ha-icon{color:var(--warning-color,#ff9800)}@media(max-width:800px){.pump-calibration{grid-template-columns:1fr 1fr}.pump-calibration ha-button{width:100%}}
  `;}
}
