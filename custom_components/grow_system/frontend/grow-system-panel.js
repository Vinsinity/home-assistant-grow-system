class GrowSystemPanel extends HTMLElement {
  ...
  _cycleSummary(){
    ...
    const today=new Date().toISOString().slice(0,10),days=this._cultivationDays(),current=Math.max(1,days.findIndex((item)=>item.date===today)+1),total=days.length;
  ...
  _calendarView(){
    const c=this._config?.cultivation||{};if(!c.start_date)return this._cycleSummary();
    const today=new Date().toISOString().slice(0,10),days=this._cultivationDays();
    return `<div class="journal-view">${this._cycleSummary()}<ha-card class="calendar-card"><div class="calendar-head"><div><span class="eyebrow">${c.name}</span><h2>Yetiştirme takvimi</h2><p>Plan yol gösterir; aşama değişiklikleri gerçek tarihleriyle ayrıca kaydedilir.</p></div><div class="calendar-legend"><span class="germination">Çimlenme</span><span class="early_veg">Erken veg</span><span class="veg">Veg</span><span class="bloom">Çiçeklenme</span><span class="darkness">Karanlık</span></div></div><div class="weekday"><span>Pzt</span><span>Sal</span><span>Çar</span><span>Per</span><span>Cum</span><span>Cmt</span><span>Paz</span></div><div class="grow-calendar">${days.map((day)=>`<button ${day.number===1?`style="grid-column:${((new Date(`${day.date}T00:00:00`).getDay()+6)%7)+1}"`:""} data-grow-day="${day.date}" class="grow-day ${day.stage} ${day.date===today?"today":""} ${day.actual&&day.actual!==day.stage?"deviation":""}"><small>Gün ${day.number}</small><b>${new Date(`${day.date}T00:00:00`).toLocaleDateString("tr-TR",{day:"numeric",month:"short"})}</b><span>${day.label}</span>${day.journal.photos?.length?`<em><ha-icon icon="mdi:image-multiple-outline"></ha-icon>${day.journal.photos.length}</em>`:""}</button>`).join("")}</div></ha-card></div>`;
  }
  _albumView(){
    const days=this._cultivationDays(),photos=days.flatMap((day)=>(day.journal.photos||[]).map((photo)=>({...photo,date:day.date,day:day.number})));
    return `<div class="journal-view">${this._cycleSummary()}<ha-card class="album-card"><div class="calendar-head"><div><span class="eyebrow">Görsel günlük</span><h2>Albüm</h2><p>Fotoğraflar çekildikleri güne ve yetiştirme aşamasına bağlı kalır.</p></div></div>${photos.length?`<div class="album-grid">${photos.map((photo)=>`<button data-grow-day="${photo.date}"><img src="${photo.url}" alt="Gün ${photo.day}"><span>Gün ${photo.day} · ${photo.date}</span></button>`).join("")}</div>`:`<div class="album-empty"><ha-icon icon="mdi:image-multiple-outline"></ha-icon><b>Albüm henüz boş</b><span>Gün detayından fotoğraf eklendiğinde burada kronolojik olarak görünecek.</span></div>`}</ha-card></div>`;
  }
  _growDayDialog(){
    if(!this._selectedGrowDay)return "";const day=this._cultivationDays().find((item)=>item.date===this._selectedGrowDay);if(!day)return "";
    return `<div class="dialog-scrim" data-day-scrim><div class="device-dialog day-dialog"><div class="dialog-head"><div><span class="eyebrow">Gün ${day.number} · ${day.date}</span><h2>${day.label}</h2></div><ha-icon-button data-close-day><ha-icon icon="mdi:close"></ha-icon></ha-icon-button></div><div class="dialog-body"><div class="day-status"><span><small>Planlanan aşama</small><b>${day.label}</b></span><span><small>Gerçek aşama</small><b>${day.actual||"Henüz kaydedilmedi"}</b></span></div><div class="day-section"><h3>Günün ölçümleri</h3><p>Bu tarih için Home Assistant Recorder geçmişi grafiklere bağlanacak.</p></div><div class="day-section"><h3>Fotoğraflar ve notlar</h3><p>${day.journal.photos?.length?`${day.journal.photos.length} fotoğraf kayıtlı.`:"Bu güne henüz fotoğraf veya not eklenmedi."}</p></div></div></div></div>`;
  }
